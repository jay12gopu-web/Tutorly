...
assert.ok(app.includes("ReasoningStatus?.stopAll?.()"), "request cancellation should clear every active reasoning status");
assert.ok(app.includes("pendingChatStartTimer"), "new-chat cancellation should clear delayed request startup");
assert.ok(app.includes('ReasoningStatus?.setStage?.(loadingMessage, "structuring")'), "normal responses should show Structuring only during response preparation");
assert.ok(app.includes('ReasoningStatus?.setStage?.(message, "structuring")'), "regenerated responses should show Structuring only during response preparation");
...
assert.ok(!app.includes("Tutorly is thinking") && !reasoningStatusSource.includes("Tutorly is thinking") && !reasoningStatusSource.includes("Tutorly is preparing"), "the live chat must show only the single-word cue");
assert.ok(!reasoningStatusSource.includes("ROTATION_DELAYS") && !reasoningStatusSource.includes("setTimeout"), "reasoning words must follow real application stages instead of a timed flow");
assert.ok(app.includes('const BOT_AVATAR_SRC = "assets/brand-star.png"'), "the response avatar should use the Tutorly star asset");
assert.ok(page.includes('<span class="sidebar-brand-mark" aria-hidden="true"><img src="assets/brand-star.png" alt="" /></span>'), "the sidebar should use the Tutorly star instead of the boxed glyph");
assert.ok(page.includes('<span aria-hidden="true"><img src="assets/brand-star.png" alt="" /></span>Tutorly'), "the header should use the Tutorly star instead of the boxed glyph");
...
assert.equal(reasoningStatus.wordFor("request", { subject: "mathematics" }), "Analyzing");
assert.equal(reasoningStatus.wordFor("request", { subject: "english", intent: "writing_help" }), "Interpreting");
assert.equal(reasoningStatus.wordFor("request", { intent: "concept_explanation" }), "Assessing");
assert.equal(reasoningStatus.wordFor("request", { model: "deep" }), "Evaluating");
assert.equal(reasoningStatus.wordFor("structuring"), "Structuring");
