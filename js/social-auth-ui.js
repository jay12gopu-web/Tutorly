(function (root) {
  ...
  const errorMessages = {
    ...
    callback_invalid: "Sign-in could not be completed. Please try again.",
    provider_failed: "Sign-in could not be completed. Please try again."
  };

  function statusNode() {
    let node = host.querySelector(".social-auth-status");
    if (!node) {
      node = document.createElement("p");
      node.className = "social-auth-status";
  ...
  async function completeOAuth(resultCode) {
    host.hidden = false;
    setStatus("Finishing secure sign-in…", false);
    try {
